...
for i = 1:length(langs)
    language = langs{i};
    path = strcat(strcat('..\..\Pagecounts_to_Ngrams\',language),'_stripped.json');
...
%     hold off;
%     grid on;
%     saveas(h, strcat(language, '_PCvsNG_comp'), 'png');
    
    h2=figure('visible','off');
    loglog(datamat(:,4),datamat(:,5), '.')
    title('Pagecounts vs NGram (Fractional Ranking)')
    xlabel('Pagecount Rank')
    ylabel('NGram Rank')
    legend(language, 'Location', 'NorthWest')
    hold off;
    grid on;
    saveas(h2, strcat(language, '_PCvsNG_frac'), 'png');
...
%     ylabel('Difference')
%     grid on;
%     saveas(i, strcat(language, '_PCvsNG_compdif'), 'png');
...
grid on;
hold off;
saveas(k2, 'PCvsNG_fracdifbelow10k', 'png');